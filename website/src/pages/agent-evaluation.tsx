...
const SEO_TITLE =
  "Agent Evaluation & LLM Evaluation for AI Apps | MLflow Agent Platform";
const SEO_DESCRIPTION =
  "Learn agent evaluation, LLM evaluation, and AI evaluation with MLflow—the comprehensive, open-source agent engineering and ops platform.";
...
const faqs: {
  ...
  },
  {
    ...
    answer: (
      <>
        ...
        outputs of another AI system. Instead of relying solely on human review
        (which is slow and expensive), LLM judges can assess thousands of
        responses for qualities like correctness, relevance, safety, and
        helpfulness.{" "}
        <Link href={MLFLOW_GENAI_DOCS_URL + "llm-evaluate/llm-as-judge/"}>
          MLflow provides pre-built judges
        </Link>{" "}
    ...
    answerText:
      "An LLM judge is a language model used to automatically evaluate the outputs of another AI system. Instead of relying solely on human review (which is slow and expensive), LLM judges can assess thousands of responses for qualities like correctness, relevance, safety, and helpfulness. MLflow provides pre-built judges for common evaluation criteria and tools to create custom judges aligned with your domain expertise and quality standards.",
  },
  {
    question:
    ...
    answer: (
      <>
        ...
        <Link href={MLFLOW_GENAI_DOCS_URL + "llm-evaluate/"}>
          MLflow provides 70+ pre-built metrics
        </Link>{" "}
  ...
  {
    question: "How do I compare different agent versions?",
    answer: (
      <>
  ...
  },
  {
    ...
    answer: (
      <>
        ...
        <Link href={MLFLOW_GENAI_DOCS_URL + "datasets/"}>
          MLflow's dataset management
        </Link>{" "}
        lets you version datasets, track which evaluations used which dataset
    ...
    answerText:
      "Start with real examples from your application. Collect production traces that represent typical usage, edge cases, and known failure modes. Add expected outputs (ground truth) where definitive answers exist, or rely on LLM judges for open-ended evaluation. MLflow's dataset management lets you version datasets, track which evaluations used which dataset versions, and incrementally add examples as you discover new failure patterns. Quality datasets are often small but representative, covering the full range of your application's expected behavior.",
  },
  {
    question: "What is the best agent evaluation tool?",
    answer:
      "The best agent evaluation tool depends on your requirements. MLflow is the leading open-source option, offering comprehensive evaluation capabilities without vendor lock-in. It supports any agent framework (LangGraph, CrewAI, AutoGen, etc.), any LLM provider (OpenAI, Anthropic, Bedrock, etc.), and provides both pre-built and custom evaluation metrics. Unlike proprietary tools, MLflow is free, gives you full control over your data, and integrates with your existing MLOps infrastructure. With 20,000+ GitHub stars, it's trusted by thousands of organizations.",
  },
  {
    question: "What LLM providers does MLflow evaluation support?",
    answer:
      "MLflow evaluation works with any LLM provider. This includes OpenAI (GPT-4, GPT-4o), Anthropic (Claude), AWS Bedrock, Google Vertex AI (Gemini), Azure OpenAI, Mistral, Cohere, Together AI, Anyscale, and local models via vLLM or Ollama. You can use different providers for your application and your evaluation judges. MLflow's provider-agnostic design means you're never locked into a single vendor and can switch providers or use multiple providers as needed.",
  },
  {
    question: "How does MLflow compare to other evaluation tools?",
    answer:
      "MLflow differentiates through openness and completeness. Unlike proprietary tools that charge per evaluation or lock you into their ecosystem, MLflow is 100% open source under Apache 2.0. It provides end-to-end evaluation: pre-built judges, custom metric APIs, dataset management, version comparison, and production monitoring. MLflow integrates with any LLM and agent framework, stores data where you choose, and is backed by the Linux Foundation rather than a single vendor. For teams prioritizing flexibility and data sovereignty, MLflow is the clear choice.",
  },
  {
    question: "Is MLflow evaluation free?",
    answer:
      "Yes. MLflow is completely free and open source under the Apache 2.0 license, backed by the Linux Foundation. All evaluation features—pre-built judges, custom scorers, dataset management, the evaluation UI, and production monitoring—are included at no cost. You can use MLflow in commercial applications without licensing fees. The only costs are your own infrastructure (which you control) and any LLM API calls for running judges. Managed MLflow is also available on Databricks and other platforms if you prefer hosted solutions.",
  },
  {
    question: "How do I get started with agent evaluation?",
    answer: (
      <>
        ...
        with pre-built scorers like Correctness and Safety. View results in the
    ...
    answerText:
      "Getting started takes minutes. Install MLflow, create an evaluation dataset from your test cases, and run mlflow.genai.evaluate() with pre-built scorers like Correctness and Safety. View results in the MLflow UI to identify failures and track improvements. As you iterate, add custom scorers for your specific quality criteria and expand your evaluation dataset. The evaluation documentation provides step-by-step guides and examples for every agent framework.",
  },
  {
    ...
    answer: (
      <>
        ...
        reasoning quality, and goal completion rate.{" "}
    ...
    answerText:
      "LLM evaluation assesses single-turn input/output pairs: given a prompt, is the response accurate, relevant, and safe? Agent evaluation is far more complex because agents take multiple steps, use tools, and make decisions. You must evaluate the entire trajectory: Did the agent choose the right tools? Did it recover from errors? Did it complete the goal efficiently? LLM evaluation uses metrics like correctness and relevance. Agent evaluation adds trajectory metrics like tool call efficiency, reasoning quality, and goal completion rate. MLflow supports both with specialized scorers for each use case.",
  },
  {
    ...
    answer: (
      <>
        ...
        MLflow provides pre-built judges for{" "}
        <Link href={MLFLOW_GENAI_DOCS_URL + "llm-evaluate/llm-as-judge/"}>
    ...
    answerText:
      "RAG (Retrieval-Augmented Generation) evaluation requires assessing both retrieval quality and generation quality. For retrieval, measure whether the right documents were retrieved (context relevance) and whether all relevant information was found (recall). For generation, evaluate whether the response is grounded in the retrieved context (faithfulness) and doesn't hallucinate facts not present in the sources (groundedness). MLflow provides pre-built judges for context relevance, faithfulness, and groundedness, plus integration with RAGAS and DeepEval metrics for comprehensive RAG evaluation.",
  },
  {
    question: "What are the top pre-built LLM judges in MLflow?",
    answer: (
      <>
        MLflow provides pre-built judges for the most common evaluation needs:{" "}
        <strong>Safety</strong> (detects harmful, toxic, or dangerous content),{" "}
        <strong>Correctness</strong> (factual accuracy against ground truth),{" "}
        <strong>RelevanceToQuery</strong> (response addresses the question),{" "}
        <strong>Groundedness</strong> (claims supported by provided context),{" "}
        <strong>RetrievalRelevance</strong> (retrieved documents match the
        query), <strong>Fluency</strong> (grammatically correct and readable),{" "}
        <strong>Coherence</strong> (logically structured response),{" "}
        <strong>ConversationalSafety</strong> (safety across multi-turn
        conversations), <strong>ToolCallEfficiency</strong> (efficient tool use
        in agents), and <strong>RoleAdherence</strong> (stays in character).
        MLflow also integrates with{" "}
        <Link href={MLFLOW_GENAI_DOCS_URL + "llm-evaluate/"}>
          DeepEval and RAGAS
        </Link>{" "}
        for 20+ additional evaluation metrics.
      </>
    ),
    answerText:
      "MLflow provides pre-built judges for the most common evaluation needs: Safety (detects harmful, toxic, or dangerous content), Correctness (factual accuracy against ground truth), RelevanceToQuery (response addresses the question), Groundedness (claims supported by provided context), RetrievalRelevance (retrieved documents match the query), Fluency (grammatically correct and readable), Coherence (logically structured response), ConversationalSafety (safety across multi-turn conversations), ToolCallEfficiency (efficient tool use in agents), and RoleAdherence (stays in character). MLflow also integrates with DeepEval and RAGAS for 20+ additional evaluation metrics.",
  },
  {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:url"
          content="https://mlflow.org/faq/agent-evaluation"
        />
        <link rel="canonical" href="https://mlflow.org/faq/agent-evaluation" />
      ...
      <div className="article-page">
        ...
        <div className="article-container">
          <h1>Agent Evaluation & LLM Evaluation</h1>

          <p>
            Agent evaluation and LLM evaluation are the practices of
            systematically measuring the quality, correctness, and safety of AI
            system outputs. For autonomous{" "}
            <Link href={MLFLOW_GENAI_DOCS_URL + "agent-evaluation/"}>
              agents
            </Link>
            , this is known as <a href="#agent-evaluation">agent evaluation</a>.
            For large language model applications, this is known as{" "}
            <a href="#llm-evaluation">LLM evaluation</a>. Agent evaluation
            assesses multi-step reasoning, tool selection, and goal completion.
            LLM evaluation focuses on response quality, relevance, and accuracy.
          </p>

          <p>
            Evaluation gives engineering teams confidence that their AI
            applications actually work well: not just whether they run, but
            whether they produce correct, safe, and useful results. As AI
            systems move from prototypes to production-critical applications,
            evaluation becomes essential for maintaining quality and enabling
            continuous improvement.
          </p>

          <p>
            Unlike traditional software testing, AI applications are{" "}
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: "24px" }}>
              ...
              <li style={{ marginBottom: "8px" }}>
                <a href="#agent-vs-llm">Agent vs LLM Evaluation</a>
              </li>
              <li style={{ marginBottom: "8px" }}>
          ...
          <h2>Why Agent Evaluation Matters</h2>
          ...
          <div className="grid-2">
            <div className="card">
              ...
              <p>
                ...
                can include hallucinations, irrelevant responses, or unsafe
                content.
              </p>
              <p>
            ...
            <div className="card">
              ...
              <p>
                ...
                decisions about tool use and reasoning that are hard to
            ...
            <div className="card">
              <h3>Confident Iteration</h3>
              <p>
                <strong>Problem:</strong> Without objective metrics, it's
                impossible to know if changes actually improve your AI
                application.
              </p>
              <p>
                <strong>Solution:</strong> Compare evaluation results across
                versions to measure improvement and make data-driven decisions
                about what to deploy.
              </p>
            </div>
          </div>
          ...
          <p>
            ...
            provides pre-built judges for common quality dimensions (safety,
          ...
          <h2 id="agent-vs-llm">
            Agent Evaluation vs LLM Evaluation: Key Differences
          </h2>
            ...
            >
              ...
              <tbody>
                ...
                <tr style={{ borderBottom: "1px solid #e2e8f0" }}>
                  ...
                  <td style={{ padding: "16px", color: "#3d3d3d" }}>
                    Tool call accuracy, goal completion, efficiency, error
                    recovery
                  </td>
                </tr>
                <tr
                ...
                <tr style={{ backgroundColor: "#fafafa" }}>
                  ...
                  <td style={{ padding: "16px", color: "#3d3d3d" }}>
                    Safety, Correctness, RelevanceToQuery, Groundedness
                  </td>
                  <td style={{ padding: "16px", color: "#3d3d3d" }}>
                    ToolCallEfficiency, RoleAdherence, ConversationalSafety,
          ...
          <h2 id="evaluation-lifecycle">The Agent Evaluation Lifecycle</h2>

          <p>
            Successful agent evaluation isn't a one-time activity—it's a
          ...
          >
            ...
            >
              ...
              >
                Run inline evaluations as you develop. Test prompt variations
            ...
            >
              ...
              >
                ...
                deployment. Establish baseline metrics.
              </div>
            </div>
          ...
          <ul>
            ...
            <li>
              ...
              expected behavior—before users notice and complain.
            </li>
            <li>
              ...
              <Link href="/faq/llm-tracing">traces</Link> reveal exactly what
              went wrong.
            </li>
            <li>
          ...
          <ul>
            ...
            <li>
              <Link
                href={MLFLOW_GENAI_DOCS_URL + "llm-evaluate/custom-scorers/"}
                style={{ color: "#007bff", fontWeight: "600" }}
              >
                Custom Scorers
              </Link>
          ...
          <p>
            ...
            against datasets using pre-built or custom scorers. Results are
            tracked in MLflow, where you can compare versions, drill into
            failures, and monitor quality over time. You can evaluate during
            development (testing new prompts), before deployment (comprehensive
            benchmark testing), and in production (continuous monitoring).
          </p>
          ...
          <p style={{ marginTop: "32px", marginBottom: "0px" }}>
            <strong>Basic Evaluation with Pre-built Judges</strong>
          </p>
...
print(f"Correctness pass rate: {results.metrics['correctness/pass_rate']}")`}
          ...
          <p style={{ marginTop: "32px", marginBottom: "0px" }}>
            <strong>Custom LLM Judge</strong>
          </p>
...
results = mlflow.genai.evaluate(
    ...
    scorers=[conversation_quality_judge],
          ...
          <p style={{ marginTop: "32px", marginBottom: "0px" }}>
            <strong>Code-based Custom Metrics</strong>
          </p>
    ...
    scorers=[response_length, contains_required_sections],
          ...
          <div className="info-box">
            <p>
              ...
              stars and 900+ contributors, it provides comprehensive evaluation
              capabilities with no vendor lock-in.{" "}
              <Link href={MLFLOW_GENAI_DOCS_URL + "eval-monitor/"}>
          ...
          <ul>
            ...
            <li>
              <Link href="/faq/llm-tracing">LLM Tracing FAQ</Link>
            </li>
            <li>
