...
import { MLFLOW_DOCS_URL, MLFLOW_GENAI_GET_STARTED_URL } from "@site/src/constants";
    ...
    <div className={cn("grid grid-cols-1 lg:grid-cols-2 gap-8")}>
      ...
      {variant !== "blue" ? (
        <div className="flex flex-col gap-8 p-8 bg-[#fff]/4 rounded-2xl @container justify-between">
          <div className="flex flex-col gap-8">
            <div className="flex flex-row justify-between items-center gap-4">
              <div className="flex flex-row justify-center items-end gap-3 flex-wrap">
                <h3 className="m-0 text-white">Free hosted MLflow </h3>
            ...
            <div className="flex flex-col gap-4">
              {[
                "Explore MLflow with a free, fully managed experience",
                "Built and maintained by the creators of MLflow, with full OSS compatibility"
              ].map((bulletPoint, index) => (
                <div key={index} className="flex flex-row items-center gap-4">
                  <Checkmark className="shrink-0" />
                  <span className="text-md font-light text-gray-600">
          ...
          <GetStartedButton size="large" width="full" variant="dark" link={MLFLOW_GENAI_GET_STARTED_URL} />
        </div>
      ) : null}
    </div>
  );
