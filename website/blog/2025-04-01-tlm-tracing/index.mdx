...
We can see the resulting traces in the MLflow UI and, if you are running this code in a Jupyter notebook, you can see the traces in the cell output.
...
We'll use the following helper function to extract the prompt and response from each trace and return three lists: request ID's, prompts, and responses. We can then construct a DataFrame with the evaluation results, sort and filter the the results by trustworthiness score, and tag the traces in MLflow with the scores and explanations.
