...
description: Trace every token, evaluate every session, and extend Claude Code with MLflow's MCP server and skills. Five tips to turn your coding agent from a black box into a controllable workflow..
slug: mlflow-claude-code
authors: [yuki-watanabe]
tags: [genai, evaluation, tracing, claude-code]
thumbnail: /img/blog/mlflow-claude-code-thumbmail-6.png
image: /img/blog/mlflow-claude-code-thumbmail-6.png
---
...
To start tracing your Claude Code sessions, run a single command in your project. Alternatively, you can set up the hook at user level by following the [guide](https://mlflow.org/docs/latest/genai/tracing/claude-code/index.html) here.
...
MLflow supports [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) for connecting Claude Code or various different coding agents and LLM frameworks directly to MLflow.
...
Another way for Claude Code to interact with services is to use Command Line Interface (CLI). While MCP provides narrower scope of tools and their interface, CLI gives Claude Code more freedom to chain multiple commands into custom workflows.
...
While the MCP server and CLI give Claude Code essential building blocks to interact with MLflow, they do not tell it how to combine them to form a workflow. [Skills](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview) is a way to package a set of instructions to teach an agent how to perform specific tasks or workflows.

MLflow provides an official Skills `mlflow/skills` that covers a wide range of tasks such as trace analysis, agent evaluation, metrics querying, and debugging. Just by installing this Skills, your coding agent becomes a full-fledged MLflow expert and helps you with your daily work.
...
Since it uses your existing Claude Code subscription, **no extra API keys and no additional cost**. Everything runs locally with full transparency. Get started by configuring the Assistant in the MLflow UI and follow [docs](https://mlflow.org/docs/latest/genai/getting-started/try-assistant.html) to customize it to your needs.
