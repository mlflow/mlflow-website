...
Claude Code is a powerful coding agent that help you write code, automate workflow, design features, and so much more. It has been disrupting the software engineering process and becoming an indispensable tool for developers.In this blog, we will show you how to enhance your Claude Code experience with MLflow and 10x your productivity as an AI engineer, using [MLflow](https://mlflow.org/), an the open-source platform for adding observability and control into AI/ML workflows.

<TOCInline toc={toc} maxHeadingLevel={2} />

## Tip 1: Monitoring Claude Code Usage Pattern
...
Here is where MLflow comes in. MLflow's [Tracing](https://mlflow.org/docs/latest/genai/tracing/index.html) capability gives you full transparency into Claude Code sessions, tools, errors, to monitor the usage pattern and diagnose issues.
...
To start tracing your Claude Code sessions, run a single command in your project. Alternatively, you can setup the hook at user level by following the [guide](https://mlflow.org/docs/latest/genai/tracing/claude-code/index.html) here.
...
### `MLflow MCP Server`
MLflow support [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) for connecting Claude Code or various different coding agents and LLM frameworks directly to MLflow.
...
Another way for Claude Code to interact with services is to use Command Line Interface (CLI). While MCP provides narrower scope of tools and their interface, CLI gives Claude Code more flexibility to run flexible workflow by chaining multiple commands together.
...
MCP and CLI are case-by-case, use the one that fits your needs best.

## Tip 4: Automating Workflow with MLflow Skills

In day-to-day work of AI engineers, there are many manual repetitive tasks such as debugging errors in traces, generating a report from evaluation results, monitoring metric trends, etc. Claude Code can help you with these tasks, but it needs to be trained to do so.

While the MCP server and CLI gives Claude Code an essential building block to interact with MLflow, they do not tell how to combine them to form a workflow. [Skills](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview) is a way to package a set of instructions to teach agent how to perform specific tasks or workflows.

MLflow provides an official Skills `mlflow/skills` that covers a wide range of tasks such as trace analysis, agent evaluation, metrics querying, and debugging. Just by installing this Skills, your coding agent becomes a full-fledged MLflow expert and help you with your daily work.
...
This adds official MLflow skills to your Claude Code, so that it can perform various tasks such as trace analysis, agent evaluation, metrics querying, and debugging.
...
We have been talking about how to use Claude Code CLI with MLflow to boost your productivity. However, what if you can run Claude Code directly **inside** MLflow?

[MLflow Assistant](https://mlflow.org/docs/latest/genai/getting-started/try-assistant.html) achieves this dream. It is a Claude Code powered assistant that is embedded directly in the MLflow UI and help you with your MLflow related tasks.

MLflow Assistant eliminates that context-switching entirely. You don't need to manually copy and paste IDs, error stacktrace, evaluation results, back and forth between the UI and the terminal. Claude Code now shares the same context of what you are seeing in the UI, such as which traces, runs, evaluations, you are looking at, and work like another AI engineer sit next to your desk.
...
Since it uses your existing Claude Code subscription, no extra API keys, no additional cost. Everything runs locally with full transparency. Get started by configuring the Assistant in the MLflow UI and follow [docs](https://mlflow.org/docs/latest/genai/getting-started/try-assistant.html) to customize it to your needs.
...
5. **Assist** from inside the MLflow UI itself — no more copy-pasting between terminal and dashboard.

Coding agents are reshaping how software gets built, and we believe the infrastructure around them should be open. MLflow is committed to becoming a **coding-agent native platform**, built in the open, for the community, with first-class support for the agentic workflows that are quickly becoming the norm. These integrations are just the beginning, and we're doubling down.
