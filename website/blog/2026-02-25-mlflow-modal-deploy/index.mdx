...
Traditional GPU deployment options require upfront provisioning: you choose an instance type, configure a container, and pay for idle compute. For teams running inference workloads that are bursty (a batch of predictions every few hours, or interactive usage during business hours), this creates a cost-efficiency problem.

Serverless GPU platforms like Modal eliminate this by scaling containers to zero when idle and spinning them up on demand. The trade-off is cold start latency, but for most ML serving use cases, sub-minute cold starts are acceptable compared to paying for 24/7 GPU instances.

MLflow's [plugin architecture](https://mlflow.org/docs/latest/plugins.html) makes it straightforward to add new deployment targets without modifying core MLflow. The `mlflow-modal-deploy` plugin registers itself as a deployment target, so the same `get_deploy_client()` API that works with SageMaker or Azure ML also works with Modal.
...
{/* [SCREENSHOT: MLflow UI showing the logged model run with artifacts. Navigate to the Experiments page, click on the run, show the Artifacts tab with the model directory visible. This establishes the "before" state -- a trained model sitting in MLflow.] */}
...
{/* [SCREENSHOT: Modal dashboard showing the deployed app with auto-scaling metrics. If you have a Modal account, deploy the model above and navigate to modal.com -> Apps -> production-model. Show the container count graph scaling up and down. This visually demonstrates the auto-scaling behavior.] */}
...
For LLM and generative models, the plugin supports Server-Sent Events (SSE) streaming through `predict_stream()`. The streaming endpoint is automatically generated alongside the standard prediction endpoint.
...
If the deployed model doesn't support streaming natively, the endpoint gracefully falls back to returning the full prediction as a single SSE chunk. This means the same `predict_stream()` API works regardless of whether the underlying model is a streaming LLM or a traditional sklearn classifier.
...
<img
  src={require("./architecture-diagram.svg").default}
...
The `mlflow-modal-deploy` plugin was developed as an independent extension of MLflow's deployment plugin architecture. Technical review was provided by the Modal team. The plugin is published on [PyPI](https://pypi.org/project/mlflow-modal-deploy/) (v0.6.0) and validated against MLflow 2.x and Modal 1.0.
