...
- **One-command GPU deployment**: `create_deployment()` takes any MLflow `pyfunc` model to a live Modal endpoint with auto-scaling, streaming, and dynamic batching across 10 GPU types (T4 through B200).
- **Automatic dependency management**: The plugin reads `requirements.txt` or `conda.yaml` from MLflow model artifacts to reproduce the exact training environment in the serving container, with support for wheel files and private registries.
- **Scale-to-zero by default**: Pay only for compute time you use. Endpoints scale from zero to many containers without managing servers, Dockerfiles, or GPU drivers.
...
If you find MLflow useful, give us a star on [GitHub](https://github.com/mlflow/mlflow) and check out the [MLflow website](https://mlflow.org/) for more.

## Related Blogs

- [Introducing DeepEval, RAGAS, and Phoenix Judges in MLflow](/blog/third-party-scorers)
- [Deterministic Safety and PII Checks in MLflow with Guardrails AI](/blog/mlflow-guardrails-scorers)
- [Agent Trace Evaluation with TruLens Scorers in MLflow](/blog/mlflow-trulens-evaluation)
