---
title: AI Observability with MLflow Tracing
description: An introduction to MLflow tracing for AI Engineers
slug: ai-tracing-intro
...
Now, when you use the OpenAI library, MLflow will capture *traces* of your model calls. For example, MLflow will log a trace of the following OpenAI invocation because we have enabled autologging.
...
Let's take a look at the trace we just logged. Start the MLflow UI from your terminal with:
...
The easiest way to organize your traces is to separate them into [*experiments*](https://mlflow.org/docs/latest/tracking.html#experiments). Each experiment has its own traces tab, which displays the traces for that experiment.
...
You can also organize your traces with [tags](https://mlflow.org/docs/latest/llms/tracing/index.html#trace-tags) and [runs](https://mlflow.org/docs/latest/llms/tracing/index.html#q-how-can-i-associate-a-trace-with-an-mlflow-run).

## Tracing Other Providers

Our quickstart example focused on OpenAI, but MLflow supports automatic tracing of [many different AI providers and frameworks](https://mlflow.org/docs/latest/llms/tracing/index.html#automatic-tracing). The approach is the same: just add the line `mlflow.<provider>.autolog` to your notebook or script.

Here are a few examples. See [here](https://mlflow.org/docs/latest/llms/tracing/index.html#automatic-tracing) for the full list of supported providers.
...
- The `StrOutputParser`, which returns the final answer to the user's quert as a string
...
Autologging is a great place to start with MLflow tracing, but you may need more flexibility in how you collect and use traces as you develop more complex GenAI applications.

- For a longer conceptual introduction to tracing, read [this guide](https://mlflow.org/docs/latest/llms/tracing/overview.html) on tracing concepts.
- MLflow traces can provide an excellent source of data for evaluation, SME review, fine-tuning, and more. Learn about searching and retrieving trace data [here](https://mlflow.org/docs/latest/llms/tracing/search-traces.html).
...
- You can add tracing to your own AI applications with the tracing [fluent APIs](https://mlflow.org/docs/latest/llms/tracing/index.html#tracing-fluent-apis) and [client APIs](https://mlflow.org/docs/latest/llms/tracing/index.html#tracing-client-apis). You can also [add tracing to libraries and frameworks](https://mlflow.org/blog/custom-tracing) that do not (yet) have autologging support.
