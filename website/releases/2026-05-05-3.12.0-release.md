---
title: MLflow 3.12.0
slug: 3.12.0
authors: [mlflow-maintainers]
---

MLflow 3.12.0 is a release focused on improving our LLM observability workflows, making tracing more accessible, feature-rich, and performant.

## 🖼️ Multimodal Tracing

<div style={{ display: "flex", gap: "1rem", alignItems: "flex-start", flexWrap: "wrap" }}>
  <img alt="Trace with image" src="/img/releases/3.12.0/trace-image-content.png" style={{ maxWidth: "48%", height: "auto", flex: "1 1 320px", minWidth: 0 }} />
  <img alt="Trace with audio" src="/img/releases/3.12.0/trace-audio-player.png" style={{ maxWidth: "48%", height: "auto", flex: "1 1 320px", minWidth: 0 }} />
</div>

Users can now store multimodal content in tracing spans as artifact attachments instead of inline binary data. We've also patched the UI to support the new mlflow-attachment:// style URI, with rich rendering available for PDFs, audio, and images.

This feature works out of the box with autologging, but manual attachment management is also possible. Visit the [documentation page](https://mlflow.org/docs/latest/genai/tracing/observe-with-traces/multimodal/) to learn more.

## 🤖 Codex, Gemini, Qwen coding agent tracing support

![Codex Tracing](/img/releases/3.12.0/codex-tracing.png)

Similar to our Claude Code tracing integration, we've now added support for the Codex, Gemini, and Qwen coding agent platforms as well! For intructions on how to get started, check out the doc pages at:

- [Codex](https://mlflow.org/docs/latest/genai/tracing/integrations/listing/codex/)
- [Gemini CLI](https://mlflow.org/docs/latest/genai/tracing/integrations/listing/gemini_cli/)
- [Qwen Code](https://mlflow.org/docs/latest/genai/tracing/integrations/listing/qwen_code/)

## 🛡️ Gateway guardrails

<video controls autoplay muted loop playsinline width="100%">
  <source
    src={require("/img/releases/3.12.0/guardrails.mp4").default}
    type="video/mp4"
  />
</video>

You can now set guardrails on your gateway endpoints to prevent unsafe or non-compliant model inputs and outputs. Try it out in the MLflow UI, and visit the [documentation page to learn more](https://mlflow.org/docs/latest/genai/governance/ai-gateway/guardrails/)!

## ⚡ Trace table pagination

<video controls autoplay muted loop playsinline width="100%">
  <source
    src={require("/img/releases/3.12.0/trace-pagination.mp4").default}
    type="video/mp4"
  />
</video>

The traces tab is now paginated, rather than fetching all traces up to a limit of 1000. This improves initial load time, and makes the page feel more responsive overall.

## Full Changelog

For a comprehensive list of changes, see the [release change log](https://github.com/mlflow/mlflow/releases/tag/v3.12.0).

## What's Next

### Get Started

Install MLflow 3.12.0 to try these new features:

```bash
pip install mlflow==3.12.0
```

### Share Your Feedback

We'd love to hear about your experience with these new features:

- [GitHub Issues](https://github.com/mlflow/mlflow/issues) - Report bugs or request features
- [MLflow Roadmap](https://github.com/mlflow/mlflow/discussions/19855) - See what's coming next and share your ideas
- ⭐ [Star us on GitHub](https://github.com/mlflow/mlflow) - Show your support for the project

### Learn More

- [Join our upcoming webinar](https://luma.com/mlflow312) to see these features in action
- Check out the [MLflow documentation](https://mlflow.org/docs/latest) for detailed guides
