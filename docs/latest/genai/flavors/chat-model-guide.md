...
When building GenAI applications in MLflow, it's essential to choose the right model abstraction that balances ease of use with the level of customization you need. MLflow offers two primary classes for this purpose: [`mlflow.pyfunc.ChatModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) and [`mlflow.pyfunc.PythonModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PythonModel). Each has its own strengths and trade-offs, making it crucial to understand which one best suits your use case.
...
This tutorial will guide you through the process of creating a custom chat agent using MLflow's [`mlflow.pyfunc.ChatModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) class.

By the end of this tutorial you will:

* Integrate [MLflow Tracing](/mlflow-website/docs/latest/genai/tracing.md) into a custom [`mlflow.pyfunc.ChatModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) instance.
* Customize your model using the `model_config` parameter within [`mlflow.pyfunc.log_model()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.log_model).
* Leverage standardized signature interfaces for simplified deployment.
* Recognize and avoid common pitfalls when extending the [`mlflow.pyfunc.ChatModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel) class.

## Prerequisites[​](#prerequisites "Direct link to Prerequisites")

* Familiarity with MLflow logging APIs and GenAI concepts.
* MLflow version 2.11.0 or higher installed for use of [`mlflow.pyfunc.ChatModel`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel).
* MLflow version 2.14.0 or higher installed for use of [MLflow Tracing](/mlflow-website/docs/latest/genai/tracing.md).
...
[MLflow Tracing](/mlflow-website/docs/latest/genai/tracing.md) allows you to monitor and log the execution of your model's methods, providing valuable insights during debugging and performance optimization.
...
The [`fluent APIs`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.html#mlflow.start_span) context handler implementation for initiating spans is useful when you need full control of the logging of each aspect of the span's data.
...
![Traces in the MLflow UI for the Agent example](/mlflow-website/docs/latest/assets/images/agent-trace-ui-83d8215ea40e973cff677b782c1e7579.png)
...
If you're working with a library that is natively supported in MLflow, leveraging the built-in support for logging and loading your implementation will always be easier than implementing a custom model. It is recommended to check the [supported GenAI flavors](/mlflow-website/docs/latest/genai/flavors.md) to see if there is a built-in solution that will meet your use case needs in one of the many integrations that are available.
...
MLflow offers the [`mlflow.models.validate_serving_input()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.models.html#mlflow.models.validate_serving_input) API to ensure that the model that you have logged is capable of being interacted with by emulating the data processing that occurs with a deployed model.
...
* **Models from code**:
  <!-- -->
  * MLflow recommends using [models from code](/mlflow-website/docs/latest/ml/model/models-from-code.md) when authoring GenAI agents or apps, for robust logging and straightforward deployment of agents containing arbitrary Python code.
...
[`mlflow.models.set_model`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.models.html#mlflow.models.set_model), passing an instance of our `BasicAgent`, to indicate which model object to use for inference when our agent is loaded back.
...
* **Validation and Deployment Readiness**: The importance of validating your model before deployment cannot be overstated. By using tools like [`mlflow.models.validate_serving_input()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.models.html#mlflow.models.validate_serving_input), you can catch and resolve potential deployment issues early, saving time and effort during the production deployment process.
