...
[`ResponsesAgent`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ResponsesAgent) is a subclass of `PythonModel` that provides a framework-agnostic way to create an agent model. Authoring an agent using `ResponsesAgent` provides the following benefits:
...
Log your agent using the [Models-from-code](/mlflow-website/docs/latest/ml/model/models-from-code.md) approach. This approach is framework-agnostic and supports all authoring frameworks:
