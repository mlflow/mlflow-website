...
First, install MLflow along with the `gateway` extras to get access to a range of serving-related dependencies, including `uvicorn` and `fastapi`. Note that direct dependencies on OpenAI are unnecessary, as all supported providers are abstracted from the developer.
