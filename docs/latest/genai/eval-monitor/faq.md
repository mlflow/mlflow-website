...
MLflow provides [`mlflow.genai.to_predict_fn()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.genai.html#mlflow.genai.to_predict_fn), which wraps a Databricks Model Serving endpoint so it behaves like a predict function compatible with GenAI evaluation.
...
See the [Migrating from MLflow 2 LLM Evaluation](/mlflow-website/docs/latest/genai/eval-monitor/legacy-llm-evaluation.md) guide.
...
![LLM Judge Cost](/mlflow-website/docs/latest/images/mlflow-3/eval-monitor/tracking-judge-cost.png)

Managing the balance between cost and accuracy is important. To use a more cost-effective LLM model while maintaining accuracy, you can leverage the [LLM Judge Alignment](/mlflow-website/docs/latest/genai/eval-monitor/scorers/llm-judge/alignment.md) feature.
...
To get metrics for an existing evaluation run, you can use [`mlflow.get_run()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.html#mlflow.get_run) API:
...
You can get the traces for the evaluation run by using [`mlflow.search_traces()`](/mlflow-website/docs/latest/api_reference/python_api/mlflow.html#mlflow.search_traces) API:
