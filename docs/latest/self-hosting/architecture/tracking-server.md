...
* **Sharing Results**: The tracking server also serves [Tracking UI](/mlflow-website/docs/latest/ml/tracking.md#tracking_ui) endpoint, where team members can easily explore each other's results.
...
If you are using remote storage, you have to configure the credentials for the server to access the artifacts. Be aware of that The MLflow artifact proxied access service enables users to have an *assumed role of access to all artifacts* that are accessible to the Tracking Server. Refer [Manage Access](/mlflow-website/docs/latest/self-hosting/architecture/artifact-store.md#artifacts-stores-manage-access) for further details.
...
For notebook integration and UI embedding options, see [Network Security](/mlflow-website/docs/latest/self-hosting/security/network.md) configuration.
